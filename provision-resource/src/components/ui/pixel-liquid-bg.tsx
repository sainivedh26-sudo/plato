...
class FluidSim {
  ...
  externalForce!: { scene: THREE.Scene; camera: THREE.Camera; mesh: THREE.Mesh };
  viscousPass!: {
    pass: ShaderPass;
  ...
  private _createPasses() {
    ...
    this.viscousPass = { pass: viscPass, output0: fbos.vel_v0, output1: fbos.vel_v1 };
  ...
  update(time: number) {
    ...
    (
      this.divergencePass.uniforms as Uniforms & { velocity: { value: THREE.Texture } }
...
export function PixelLiquidBg({
  ...
  }, [darkPalette, lightPalette, pixelSize, resolution, mouseForce, cursorSize, autoDemo]);
